describe('subgraph', function() {
    ...
    describe('with one graph pattern', function() {
        beforeEach(function() {
        ...
        describe('instantiated from graph data', function() {
            ...
            describe('subgraph', function() {
                var subgraph;
                beforeEach(function() {
                    subgraph = graph.subgraph(['a','b','c'], ['e','f']);
                });
                it('has nodes a,b,c', function() {
                    expect(subgraph.nodes().map(get_key)).toEqual(['a','b','c']);
                });
                it('has edges e,f', function() {
                    expect(subgraph.edges().map(get_key)).toEqual(['e','f']);
                });
                it('nodes b,c have subgraph', function() {
                    expect(subgraph.node('b').graph()).toBe(subgraph);
                    expect(subgraph.node('c').graph()).toBe(subgraph);
                });
                it('edges e,f have subgraph', function() {
                    expect(subgraph.edge('e').graph()).toBe(subgraph);
                    expect(subgraph.edge('f').graph()).toBe(subgraph);
                });
                it('a has outs e,f', function() {
                    expect(subgraph.node('a').outs().map(get_key)).toEqual(['e','f']);
                });
                it('c has no outs', function() {
                    expect(subgraph.node('c').outs().map(get_key)).toEqual([]);
                });
                it('a has no ins', function() {
                ...
                it('b has in e', function() {
                    expect(subgraph.node('b').ins().map(get_key)).toEqual(['e']);
                });
                it('f has source a', function() {
                    expect(subgraph.edge('f').source().key()).toEqual('a');
                });
                it('does not have edge g', function() {
                    expect(subgraph.edge('g')).toBeFalsy();
                });
                it('node c has data', function() {
